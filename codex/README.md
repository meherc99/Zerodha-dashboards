...
### Auto-generate access token (recommended)

Use the helper script to open Kite login in your browser, capture `request_token` from redirect automatically, exchange it, and write `<ALIAS>_ACCESS_TOKEN` into `.env`.

```bash
python get_access_token.py --alias Self
```

For another alias:

```bash
python get_access_token.py --alias Spouse
```

Important:
- In Kite app settings, set Redirect URL to `http://127.0.0.1:8765/callback` (or run the script with matching `--port` / `--callback-path`).
- The script requires `<ALIAS>_API_KEY` and `<ALIAS>_API_SECRET` already set in `.env`.
