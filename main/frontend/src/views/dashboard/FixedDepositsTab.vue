    ...
    <template v-if="hasFDHoldings">
      ...
      <section class="deposit-insights" aria-label="Fixed deposit highlights">
        ...
        <article>
          ...
          <strong>{{ averageRate.toFixed(2) }}% p.a.</strong>
          <small>{{ fdHoldings.length }} total deposits</small>
        </article>
        <article>
          <span>Bank relationships</span>
          <strong>{{ bankCount }}</strong>
          <small>{{ fdHoldings.length }} {{ fdHoldings.length === 1 ? 'deposit' : 'deposits' }}</small>
        </article>
        <article>
      ...
      <div class="charts-grid">
        ...
        <ChartPanel
          ...
          subtitle="Annual interest rate per deposit"
          :has-data="rateComparisonData.labels.length > 0"
        >
...
const averageRate = computed(() => {
  if (!fdHoldings.value.length) return 0
  const total = fdHoldings.value.reduce((sum, h) => sum + parseRate(h), 0)
  return total / fdHoldings.value.length
})
...
const rateComparisonData = computed(() => {
  const deposits = fdHoldings.value
    .slice()
    .sort((a, b) => parseRate(b) - parseRate(a))
    .slice(0, 10)
  return {
