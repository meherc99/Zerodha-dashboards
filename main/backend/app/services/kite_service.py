    ...
    def get_equity_holdings(self) -> List[Dict]:
        ...
        try:
            ...
            for holding in holdings:
                ...
                close_price = self._decimal(holding.get('close_price'))
                ...
                # day_change and day_change_percentage come directly from Kite
                # holdings response; fall back to computing from close_price if absent.
                day_change = self._decimal(holding.get('day_change'))
                day_change_pct = self._decimal(holding.get('day_change_percentage'))
                if not day_change and close_price:
                    day_change = last_price - close_price
                if not day_change_pct and close_price:
