...
class BankAnalyticsService:
    ...
    @staticmethod
    def get_monthly_cashflow(bank_account_id, num_months, user_id):
        """
        Get monthly income vs expenses for the last N calendar months.

        Args:
            bank_account_id: ID of the bank account
            num_months: Number of calendar months to analyze (default: 12)
            user_id: ID of the requesting user

        Returns:
            {
                'months': ['Jan 2025', 'Feb 2025', ...],
                'income': [50000.0, 45000.0, ...],
                'expenses': [30000.0, 32000.0, ...],
                'net': [20000.0, 13000.0, ...],
                'num_months': 12
            }
        """
        ...
        today = date.today()
        # Build list of (year, month) for the last num_months months (oldest first)
        month_keys = []
        ...
        monthly_totals = (
            db.session.query(
                sa_extract('year', Transaction.transaction_date).label('year'),
                sa_extract('month', Transaction.transaction_date).label('month'),
                Transaction.transaction_type,
                func.sum(Transaction.amount).label('total'),
            )
            .filter(
            ...
            .group_by(
                sa_extract('year', Transaction.transaction_date),
                sa_extract('month', Transaction.transaction_date),
                Transaction.transaction_type,
            )
            .all()
        )
        ...
        for row in monthly_totals:
            ...
            if key not in data_map:
                data_map[key] = {'credit': 0.0, 'debit': 0.0}
            data_map[key][row.transaction_type] += float(row.total or Decimal('0'))
        ...
        for year, month in month_keys:
            ...
            vals = data_map.get((year, month), {'credit': 0.0, 'debit': 0.0})
            credit = round(vals['credit'], 2)
            debit = round(vals['debit'], 2)
            month_labels.append(label)
            income_list.append(credit)
            expenses_list.append(debit)
            net_list.append(round(credit - debit, 2))
